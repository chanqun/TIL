...
blocking 하지 않으면 다른 thread에서 작동하기 때문에 값이 생각한 것처럼 나오지 않음


## assertXXXXX 함수를 이용한 테스트 검증(1)

- 테스트 용도로 사용되는 소비자 클래스이다.
- assertXXXX 함수를 이용해 통지된 데이터를 검증할 수 있다.
- await 함수를 이용해서 지정된 시간 동안 대기하거나 완료 또는 에러 이벤트가 발생할 떄까지 대기할 수 있다.
- 완료, 에러, 구동 해지 등의 이벤트 발생 결과 값을 이용해서 데이터를 검증할 수 있다.


- assertEmpty
  - 테스트 시점까지 통지받은 데이터가 없다면 테스트에 성공한다.
  - Observable.empty()로 생성 시 완료 통지를 하기떄문에 테스트가 실패한다.
  - 통지 이벤트 자체가 없는지를 테스트할 수 있다.
...
- assertValues
  ...
  - 통지된 데이터의 값과 순서가 파라미터로 입력된 데이터의 값관 순서와 일치하면 성공
...
- assertResult
  ...
  - assertValues와의 차이점은 해당 시점까지 완료 통지를 받았는냐 받지 않았느냐이다.


- assertError
  - 해당 시점까지 에러 통지가 있으면 테스트에 성곤한다.
  - 단순히 에러 통지가 있었는지의 여부와 구체적으로 발생한 에러가 맞는지를 테스트할 수 있다.


- assertComplete
  - 해당 시점까지 완료 통지가 있으면 테스트가 성공
