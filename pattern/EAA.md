...
비동기 메시지 기반 방식이 웹 서비스를 가장 잘 활용하는 방법
...
#### 테이블 데이터 게이트웨이
테이블 데이터 게이트웨이(Table Data Gateway)는 단일 테이블이나 뷰에 접근(선택, 삽입, 갱신, 삭제)하는 모든 SQL을 포함

#### 행 데이터 게이트웨이
행 데이터 게이트웨이(Row Data Gateway)는 레코드 구조의 레코드와 완전히 동일하게 생겼지만 사용하는 프로그래밍 언어의 일반적인 메커니즘으로 접근할 수 있는 객체를 제공

#### 활성 레코드
객체에는 데이터와 동작이 모두 포함된다. 이 데이터는 대부분 지속적이며 데이터베이스에 저장

#### 데이터 매퍼
데이터 매퍼(Data Mapper)는 인메모리 객체를 데이터베이스로부터 분리하는 소프트웨어 계층
데이터 매퍼가 있으면 인메모리 객체는 심지어 데이터베이스 가 있다는 것조차 알 필요가 없다

### 11장 객체-관계형 동작 패턴
#### 작업 단위

작업 단위는 한 비즈니스 트랜잭션 중에 데이터베이스에 영향을 미치는 모든 변경 내용을 추적, 작업이 완료되면 작업 결과를 데이터베이스에 기록하기 위해 해야 하는 모든 일을 알아냄


작업 단위를 사용하면 프로그래머가 변경 내용을 직접 추적하거나 참조 무결성을 고려해 작업 순서를 조정할 필요가 없다.

AOP 를 이용해서 후처리하는 방법도 있음 -> 바이트코드에 삽입.

#### 식별자 맵

모든 객체를 한 맵에 로드해 각 객체가 한 번씩만 로드되게 한다. (Jpa 1차 캐시)

#### 지연 로드
필요한 데이터를 모두 포함하지는 않지만 데이터가 필요할 때 가져오는 방법을 아는 객체

지연 초기화 : 동등 메서드를 재정의하고 이를 식별자 메서드 대신 사용해야 한다.
...
#### 직렬화 LOB
객체 그래프를 데이터베이스 필드에 기록할 수 있는 하나의 큰 객체로 직렬화해 저장한다.
...
프론트 컨트롤러는 모든 요청을 단일 처리기 객체로 집중하는 방법으로 요청을 통합 처리
객체는 공통적인 동작을 수행하고 데코레이터를 사용에 런타임에 수정할 수 있다.
...
데이터 전송 객체는 한 번의 메서드 호출로 두 프로세스 간에 다수의 데이터 항목을 전송할 때 사용
...
비관적 오프라인 잠금은 세션 충돌의 가능성이 높다고 간주하고 시스템의 동시성을 제한 하는 반면,
낙관적 오프라인 잠금은 이러한 충돌 가능성이 낮다고 간주하므로 여러 사용자가 동시에 동일한 데이터를 가지고 작업하도록 허용할 수 있다.
...
굵은 입자 잠금(Coarse-Grained Lock)은 여러 객체를 다룰 수 있는 단일 잠금이다. 굵은 입자 잠금을 사용하면 잠금 동작 자체를 간소화할 수 있으며, 그룹을 잠그기 위해 모든 멤버를 로드할 필요도 없어진다.
